...
export function Faq({ showAll }) {
  ...
  const hiddenClass = "opacity-0 max-h-0 overflow-hidden transition-all duration-500 ease-in-out";
  const shownClass = "opacity-100 max-h-full transition-all duration-500 ease-in-out";
  

  return (
    <div className={`${containerClass} relative pb-3 border-b-4 overflow-hidden`}>
    <Accordion type="single" collapsible className="w-full">
        {allFaqs.map((faq, index) => (
          <AccordionItem value={faq.id} className={showAll || index < 5 ? shownClass : hiddenClass} key={faq.id}>
